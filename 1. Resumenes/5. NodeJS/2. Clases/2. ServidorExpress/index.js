...
const productos = [
    { id: 1, nombre: "Lavadora", marca: "Bosch", ref: "0101020023L" }
]
...
app.get("/productos/:id", (req, res) => {
    ...
    // Opción 1: Con doble igual (==)
    // const objetoEncontrado = productos.find(p => p.id == req.params.id);
    // Opción 2: Con triple igual y convertir el valor de req a número
    const objetoEncontrado = productos.find(p => p.id === Number(req.params.id));

    if (objetoEncontrado != undefined) {
        res.json(objetoEncontrado);
    } else {
        // Tiene que devolver siempre un objeto
